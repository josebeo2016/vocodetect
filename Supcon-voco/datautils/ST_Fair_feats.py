...
class Dataset_for(Dataset):
    def __init__(self, args, list_IDs, labels, base_dir, algo=5, vocoders=[], 
                 ...
                 aug_dir=None, online_aug=False, repeat_pad=True, is_train=True, spoof_category='acoustic', mode='E2E', ratio=1.0, feat_dir=None):
        """
        Args:
            ...
            mode: E2E, 2stage
            ratio: amount of samples to use from the list_IDs
        """
        self.args = args
        ...
        self.spoof_category = spoof_category
        ...
        self.classes = self.spoof_train[spoof_category].unique()
        if mode == 'E2E':
            print("Mode: E2E")
            print("label_map:", self.label_map)
            # print("Labels:", self.spoof_df[spoof_category].unique())
    ...
    def __getitem__(self, idx):
        ...
        for i in range(len(self.list_IDs[idx])):
            real_audio_file = os.path.join(self.feat_dir, self.list_IDs[idx][i] + ".npy")
            real_audio = np.load(real_audio_file)
        ...
        # Add num_additional_spoof spoof samples per class
        additional_spoofs = []
        # augmented_additional_spoofs = []
        for c in self.classes:
            if c not in self.spoof_df[self.spoof_category].unique():
                continue
            spoof_idxs = self.spoof_df[self.spoof_df[self.spoof_category] == c].sample(n=self.num_additional_spoof)
            for _, row in spoof_idxs.iterrows():
                _label = row['label']
                _path = row['path']
                additional_spoof_file = os.path.join(self.feat_dir, _path + ".npy")
                additional_spoof = np.load(additional_spoof_file)
                ...
                label += [_label]
        # print("additional_spoofs:", len(additional_spoofs))
        # print("augmented_additional_spoofs:", len(augmented_additional_spoofs))
        ...
        # merge all the data
        batch_data = real_audios  + additional_spoofs
