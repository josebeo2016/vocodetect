...
import logging
import random
from datautils.dataio import pad, load_audio, multiview_process_audio
# augwrapper
...
class Dataset_for(Dataset):
    def __init__(self, args, list_IDs, labels, base_dir, algo=5, vocoders=[], 
                 ...
                 aug_dir=None, online_aug=False, repeat_pad=True, is_train=True, spoof_category='acoustic', mode='E2E'):
        """
        Args:
        ...
        self.spoof_train = pd.read_csv(os.path.join(base_dir, 'scp/spoof_train.lst'), sep=",", header=None, names=['path', 'acoustic', 'vocoder', 'vocoder2'])
        self.spoof_dev = pd.read_csv(os.path.join(base_dir, 'scp/spoof_dev.lst'), sep=",", header=None, names=['path', 'acoustic', 'vocoder', 'vocoder2'])
        ...
        classes = all_spoof[spoof_category].unique()
        for i, c in enumerate(classes):
            self.label_map[c] = i+1
        
        if not is_train:
            # select the spoof samples from the spoof_dev list
            # with class exist in the spoof_train list
            # for calculating dev accuracy
    ...
    def __getitem__(self, idx):
        ...
        for _, row in additional_spoof_idxs.iterrows():
            ...
            audio_paths.append(additional_spoof_file)
            additional_spoof = load_audio(additional_spoof_file)
            additional_spoofs.append(additional_spoof)
            label += [_label]
            
        # merge all the data
        batch_data_ori = real_audios + additional_spoofs
        augmethod_index = 0 # use only first augmentation method
        padding_type = 'repeat' if self.repeat_pad else 'zero'
        multiview_tensor = multiview_process_audio(batch_data_ori, audio_paths, self.args, trim_length=self.trim_length, 
                                                   sr=self.sample_rate, augmentation_func=globals()[self.augmentation_methods[augmethod_index]], 
                                                   padding_type=padding_type)

        batch_data = Tensor(multiview_tensor) # [num_samples, num_views, trim_length]
        # convert to [trim_length, num_samples, num_views]
