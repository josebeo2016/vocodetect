...
class Dataset_for(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        aug_index = random.choice(range(len(self.augmentation_methods)))
        for vf in vocoded_audio_files:
            vocoded_audio = self.load_audio(vf)
            vocoded_audios.append(np.expand_dims(vocoded_audio, axis=1))
            # Augmented vocoded samples as negative data with random augmentation method (instead of first one)
            augmented_vocoded_audio = globals()[self.augmentation_methods[aug_index]](vocoded_audio, self.args, self.sample_rate, audio_path = vf)
