...
class Dataset_for(Dataset):
    def __init__(self, args, list_IDs, labels, base_dir, algo=5, augmentation_methods=[], 
        ...
        self.num_aug_real = num_aug_real
    ...
    def __getitem__(self, idx):
        ...
        augmented_audios = []
        random_aug_methods = np.random.choice(self.augmentation_methods, self.num_aug_real, replace=False)
        for augment in random_aug_methods:
        ...
        # augmented additional spoof audio samples as negative data
        augmented_spoof_audios = []
        for i in range(self.num_additional_spoof):
            # random select a augmentation methods
            random_aug_methods = np.random.choice(self.augmentation_methods, self.num_aug_spoof, replace=False)
            for augment in random_aug_methods:
                tmp = np.expand_dims(globals()[augment](np.squeeze(additional_spoofs[i],axis=1), self.args, self.sample_rate, audio_path = os.path.join(self.spoof_dir, additional_spoof_idxs[i])),axis=1)
                augmented_spoof_audios.append(tmp)
        
        # merge all the data
        batch_data = [np.expand_dims(real_audio, axis=1)] + augmented_audios + additional_audios + additional_spoofs + augmented_spoof_audios
        ...
        # label is 1 for anchor and positive, 0 for spoof
        label = [1] * (len(augmented_audios) +len(additional_audios) + 1) + [0] * (len(additional_spoofs) + len(augmented_spoof_audios))
