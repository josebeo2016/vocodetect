...
class Dataset_for(Dataset):
    def __init__(self, args, list_IDs, labels, base_dir, algo=5, vocoders=[], neural_codecs=[], 
        ...
        self.neuralcodec_dir = os.path.join(base_dir, 'neuralcodec')
        ...
        self.neural_codecs = neural_codecs
    ...
    def __getitem__(self, idx):
        ...
        # Its neural codec samples as positive data
        neural_codec_audios = []
        for nc in self.neural_codecs:
            neural_codec_file = os.path.join(self.neuralcodec_dir, nc + "_" + self.list_IDs[idx])
            neural_codec_audio = self.load_audio(neural_codec_file)
            neural_codec_audios.append(np.expand_dims(neural_codec_audio, axis=1))
        ...
        # merge all the data
        batch_data = [np.expand_dims(real_audio, axis=1)] + neural_codec_audios + augmented_audios + additional_audios + vocoded_audios + augmented_vocoded_audios
        ...
        label = [1] * (len(augmented_audios) + len(neural_codec_audios) +len(additional_audios) + 1) + [0] * len(self.vocoders*2)
        # print("label", label)
