...
class Dataset_for(Dataset):
    def __init__(self, args, list_IDs, labels, base_dir, algo=5, vocoders=['hifigan', 'hn-sinc-nsf', 'hn-sinc-nsf-hifi', 'waveglow'], augmentation_methods=[], num_additional_real=2, trim_length=64000, wav_samp_rate=16000):
        """
        Args:
        ...
        self.base_dir = os.path.join(base_dir, 'train_dev')
    ...
    def __getitem__(self, idx):
        ...
        real_audio_file = os.path.join(self.base_dir, self.list_IDs[idx] + '.wav')
        real_audio = self.load_audio(real_audio_file)

        # Vocoded audio samples as negative data
        vocoded_audio_files = [os.path.join(self.base_dir, vf + "_" + self.list_IDs[idx] + '.wav') for vf in self.vocoders]
        # print(vocoded_audio_files)
        vocoded_audios = []
        ...
        additional_audios = [np.expand_dims(self.load_audio(os.path.join(self.base_dir, self.list_IDs[i] + '.wav')),axis=1) for i in additional_idxs]
        
        # debug: 
        # print("real audio: ", real_audio.shape)
        # print("augmented audios: ", augmented_audios)
        # print("additional audios: ", additional_audios)
        # print("vocoded audios: ", vocoded_audios)
