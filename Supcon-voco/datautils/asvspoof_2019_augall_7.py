...
class Dataset_for(Dataset):
    def __init__(self, args, list_IDs, labels, base_dir, algo=5, vocoders=[], 
                 augmentation_methods=[], num_additional_real=2, num_aug_real=1, 
                 num_additional_spoof=2, num_aug_spoof=1,
    ...
    def __getitem__(self, idx):
        ...
        # merge all the data
        batch_data = [np.expand_dims(real_audio, axis=1)] + augmented_audios + additional_audios + augmented_additional_audios + vocoded_audios + augmented_vocoded_audios + additional_spoofs
        ...
        label = [1] * (len(augmented_audios) +len(additional_audios) + len(augmented_additional_audios) + 1) + [0] * (len(vocoded_audios) + len(augmented_vocoded_audios) +  len(additional_spoofs))
        # print("label", label)
