...
class Dataset_for(Dataset):
    def __init__(self, args, list_IDs, labels, base_dir, algo=5, vocoders=[], aitools=[], num_aitools=1,
        ...
        self.vocoded_dir = os.path.join(base_dir, 'vocoded')
        self.aitool_dir = os.path.join(base_dir, 'ai-processed')
        ...
        print("aitools:", aitools)
        self.num_aitools = num_aitools
    ...
    def __getitem__(self, idx):
        ...
        # AI processed audio samples as positive data - Hard positives samples :)))
        ai_processed_audio_files = [os.path.join(self.aitool_dir, ait, self.list_IDs[idx].split("/")[-1]) for ait in self.aitools]
        ai_processed_audios = []
        selected_tools = np.random.choice(self.aitools, self.num_aitools, replace=False)
        for ait in selected_tools:
            ai_processed_audio = load_audio(ait)
            ai_processed_audios.append(np.expand_dims(ai_processed_audio, axis=1))
        ...
        # merge all the data
        batch_data = [np.expand_dims(real_audio, axis=1)] + augmented_audios + additional_audios + ai_processed_audios + vocoded_audios + augmented_vocoded_audios + additional_spoofs
        ...
        label = [1] * (len(augmented_audios) +len(additional_audios) +len(ai_processed_audios) + 1) + [0] * (len(self.vocoders*2) +  len(additional_spoofs))
