...
class Dataset_for(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Random choost the set of augmentations to be applied
        augmentation_methods = np.random.choice(self.augmentation_methods, num_augmentations, replace=False)
        ...
        for vf in vocoded_audio_files:
            ...
            aug_method_idx = 0
            augmented_vocoded_audio = globals()[augmentation_methods[aug_method_idx]](vocoded_audio, self.args, self.sample_rate, audio_path = vf)
        ...
        num_augmentations = len(augmentation_methods)
        ...
        for i in additional_idxs:
            ...
            augment_idx = random.randint(0, len(augmentation_methods))
            if augment_idx<len(augmentation_methods):
                # apply augmentation method when the index is less than the length of augmentation methods
                additional_audio = globals()[augmentation_methods[augment_idx]](additional_audio, self.args, self.sample_rate, audio_path = real_audio_file)
        ...
        for i in additional_spoof_idxs:
            ...
            if augment_idx<len(augmentation_methods):
                # apply augmentation method when the index is less than the length of augmentation methods
                additional_spoof = globals()[augmentation_methods[augment_idx]](additional_spoof, self.args, self.sample_rate, audio_path = real_audio_file)
