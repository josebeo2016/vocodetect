...
class HardNegativeMixup():
    def __init__(self, device, mix_alpha=0.6, random_mix=False, tau=0.07, is_norm=True, n_synthetic=2, mixup_type='undefined'):
    ...
    def mixup(self, feats, labels):
        '''
        feats: tensor, (batch, frame_num, feat_dim) or (batch, emb_dim, 1)
        ...
        for i in range(self.n_synthetic):
            ...
            if self.is_norm:
                synthetic_sample = F.normalize(synthetic_sample)
...
class Model(nn.Module):
    ...
    def loss(self, output, feats, emb, labels, config, info=None):
        ...
        harder_mixup = HardNegativeMixup(self.device, mix_alpha=config['mixup']['mix_alpha'], 
                                   random_mix=config['mixup']['random_mix'], tau=config['mixup']['tau'], 
                                   is_norm=config['mixup']['is_norm'], n_synthetic=config['mixup']['n_synthetic'], 
                                   mixup_type=config['mixup']['mixup_type'])
