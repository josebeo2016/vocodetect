...
from model.RawNet3.RawNetBasicBlock import Bottle2neck, PreEmphasis


class RawNet3(nn.Module):
    def __init__(self, block=Bottle2neck,model_scale=8, context=True, summed=True, C=1024, **kwargs):
        super().__init__()
    ...
    def forward(self, x):
        ...
        #     x = self.bn6(x)
        # 
        return x.transpose(2,1), w.transpose(2,1) # (B, L, H)
...
if __name__ == "__main__":
    ...
    model.eval()
    input_data = torch.randn(5, 48000).to(device)
    emb, attention_w = model(input_data)
