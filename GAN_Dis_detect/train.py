...
def evaluate_accuracy(dev_loader, model, device):
    ...
    num_total = 0.0
    model.eval()
    for batch_x, _, batch_y in dev_loader:
...
def train_epoch(train_loader, model, lr,optim, device):
    ...
    for batch_x, _, batch_y in train_loader:
        ...
        batch_y = batch_y.view(-1).type(torch.int64).to(device)
        batch_out, recons, mus, logvars = model(batch_x)
        
        batch_loss = (1 - model.vae_rate)*criterion(batch_out, batch_y) + model.vae_rate*model.loss_function(recons, batch_x, mus, logvars)
...
if __name__ == '__main__':
    ...
    model = VocoDetect(parser1['model'], device)
    ...
    if args.eval_2021:
        ...
        eval_set=Dataset_ASVspoof2021_eval(list_IDs = file_eval,base_dir = os.path.join(args.database_path+'/../ASVspoof2021_{}_eval/'.format(args.track)),feat_type=parser1['dataset']['feat_type'])
    ...
    if args.eval_2019:
        ...
        print('no. of eval trials',len(file_eval))
        eval_set=Dataset_ASVspoof2019_eval(list_IDs = file_eval,base_dir = os.path.join(args.database_path+'ASVspoof2019_{}_eval/'.format(args.track)),feat_type=parser1['dataset']['feat_type'])
    ...
    train_set=Dataset_ASVspoof2019_train(list_IDs = file_train,labels = d_label_trn,base_dir = os.path.join(args.database_path+'ASVspoof2019_{}_train/'.format(args.track)),feat_type=parser1['dataset']['feat_type'])
    ...
    dev_set = Dataset_ASVspoof2019_train(list_IDs = file_dev,
        ...
        feat_type=parser1['dataset']['feat_type'])
