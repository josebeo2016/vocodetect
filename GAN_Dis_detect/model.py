...
# your task
# class VocoDetect(nn.Module):
#     def __init__(self, d_args, device):
#         super(VocoDetect, self).__init__()
...
CLASS_MAP = {
    'bioEncoderConv': bioEncoderConv,
    'bioEncoderRNN': bioEncoderRNN,
    'bioEncoderTransformer': bioEncoderTransformer,
    'bioEncoderlight': bioEncoderlight,
    'bioEncoderRNNsmall': bioEncoderRNNsmall,
    'bioEncoderTransformersmall': bioEncoderTransformersmall
}
...
class VocoDetect(nn.Module):
    def __init__(self, d_args, device):
        super(VocoDetect, self).__init__()
        self.device=device
        self.vae_rate = d_args['vae_rate']
        
        # self.encoder = CLASS_MAP[d_args['name']](d_args, device)
        
        # self.encoder = torch.nn.Sequential()
        # for i in range(d_args['n_enc_layers']):
        #     self.encoder.add_module("linear_{}".format(i), nn.Linear(d_args['enc'][i][0], d_args['enc'][i][1]))
        #     self.encoder.add_module("relu_{}".format(i),nn.ReLU(True))
        #     self.encoder.add_module("bn_{}".format(i),nn.BatchNorm1d(num_features = d_args['enc'][i][1]))
        
        
        self.decoder = torch.nn.Sequential()
        for i in range(d_args['n_dec_layers']):
            self.decoder.add_module("linear_{}".format(i), nn.Linear(d_args['dec'][i][0], d_args['dec'][i][1]))
            self.decoder.add_module("relu_{}".format(i),nn.ReLU(True))
        
        self.fc = nn.Linear(d_args['dec'][0][0], d_args['nb_classes'])
        self.sig = nn.Sigmoid()
        self.logsoftmax = nn.LogSoftmax(dim=1)
        
    def encode(self, x):
        # x = x.transpose(2,1) # [B, channel, T]
    ...
    def forward(self, x, bio = None, bio_lengths = None, y = None):
        ...
        recon_x = self.sig(recon_x)
        
        output = self.fc(z)
        output=self.logsoftmax(output)
      
        return output, recon_x, mu, logvar
