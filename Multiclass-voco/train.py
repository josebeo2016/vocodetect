...
def evaluate_accuracy(dev_loader, model, device):
    ...
    num_correct = 0.0
    model.eval()
    weight = torch.FloatTensor([0.2,1,1,1,1]).to(device)
...
if __name__ == '__main__':
    ...
    #evaluation 
    if args.eval:
        file_eval = genList(dir_meta =  os.path.join(args.database_path+'/protocol_test.txt'),is_train=False,is_eval=True)
    ...
    d_label_trn,file_train = genList( dir_meta =  os.path.join(args.train_path,'protocol.txt'),is_train=True,is_eval=False,is_dev=False)
    ...
    d_label_dev,file_dev = genList(dir_meta = os.path.join(args.train_path,'protocol.txt'),is_train=False,is_eval=False, is_dev=True)
